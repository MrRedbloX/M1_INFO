  ...
  public static void main(String[] args) throws IOException, URISyntaxException{
    ...
    if ((args == null) || args.length==0){ // on prend des chemins par défaut
      args= new String[]{
          ...
          //"/data/wikipedia/frwiki-pages-100000-articles.xml",//    100 000 pages
          //"frwiki-links-100000-articles",                    //  7 595 791 links
          //"/data/wikipedia/frwiki-pages-articles.xml",       //  ????????? pages
          //"frwiki-links-articles",                           // ?????????? links
      };
    ...
    //conf.setInt("mapred.reduce.tasks",10); // 10 taches reduce
    //conf.setInt("mapred.map.max.attempts",1); // pas de relance d'une tache map qui échoue 
    //conf.setInt("mapred.reduce.max.attempts",1); // pas de relance d'une tache reduce qui échoue 
    //conf.setBoolean("mapred.map.tasks.speculative.execution",true); // exécution spéculative des taches map
    //conf.setBoolean("mapred.reduce.tasks.speculative.execution",true); // exécution spéculative des taches reduce
