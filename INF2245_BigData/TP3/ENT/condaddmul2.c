...
int simd_condaddmul(short int aa[], short int bb[], short int cc[]) {
    ...
    int i;
    for (i = 0; i < N; i += 8) {// Roll out loop by eight to fit the eight-element vectors:
...
int simd_condaddmul2(short aa[], short bb[], short cc[]){
    ...
    int i;
    for (i = 0; i < N; i += 16) {// Roll out loop by eight to fit the eight-element vectors:
...
int main(int argc,char *argv[]){
    ...
    display_stats("SIMD SSE stats",nb_loops);
    reset_stats();
    for(i=0; i<nb_loops; i++){
        start_timer();
        simd_condaddmul2(vector_A1,vector_B,vector_C);
        pause_timer();
    }

    display_stats("SIMD AVX stats",nb_loops);
